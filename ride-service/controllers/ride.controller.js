...
export const requestRideTaxiHandler = async (req, res) => {
  try {
    ...
    const passengerId = req.user.userId || req.user.driverId;
    ...
    res.status(201).json({ message: "Поездка(такси) успешно создана", ride });
  } catch (error) {
...
export const requestRideCourierHandler = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Поездка(доставка) успешно создана", ride });
  } catch (error) {
    logger.error("Ошибка при создании поездки", {
      error: error.message,
      correlationId: req.correlationId,
...
export const changeDriverModeHandler = async (req, res) => {
  const { mode } = req.body
  const driverId = req.user.driverId

  try {
    if (!["taxi", "courier"].includes(mode)) {
      throw new Error("Допустимые значения для поля mode (taxi, courier)")
    }

    const result = await changeDriverMode(driverId, mode)

    res.status(200).json(result);
  } catch (error) {
    logger.error("Ошибка при смене мода водителя", {
      error: error.message,
      correlationId: req.correlationId,
    });

...
export const changeDriverMode = async (driverId, mode) => {
  try {
    const { mode: currentDriverMode } = await Driver.findByPk(driverId, { attributes: ['mode']} )

    if (currentDriverMode === mode) {
      throw new Error("Вы уже находитесь в этом режиме поездки")
    }

    const [updateDriverMode] = await Driver.update(
      { mode },
      {
        where: {
          id: driverId
    ...
    if (updateDriverMode === 0) {
      throw new Error("Ошибка при обновлении режима поездки")
    }

    return `Вы успешно изменили режим поездки на ${mode === "taxi" ? "такси" : mode === "courier" ? "доставка": "НЕИЗВЕСТНО"}`
  } catch (error) {
    throw error;
  }
}
...
export const requestRide = async (
  ...
  try {
    const result = await sequelize.transaction(async (transaction) => {
      ...
      const ride = await Ride.create(
        {
          ...
          mode: rideOptions.mode,
          fromAddress: rideOptions.fromAddress,
          toAddress: rideOptions.toAddress,
          comment: rideOptions.comment
        },
        { transaction }
      );
      ...
      startDriverSearch(ride.id, latitude, longitude, rideOptions.mode, correlationId);
...
const startDriverSearch = async (
  ...
  try {
    ...
    const nearbyDrivers = await findNearbyDrivers(latitude, longitude, mode, 10, 10);
...
export const getRideInfo = async (origin, destination, mode, correlationId) => {
  try {
...
export const acceptRide = async (rideId, driverId, correlationId) => {
  ...
  try {
    ...
    const { mode: currentDriverMode } = Driver.findByPk(driverId, {
      attributes: ["mode"]
    })

    if (ride.mode !== currentDriverMode) {
      throw new Error(`Вы не можете принять поездки, так как вы в другом режиме`); 
    }
